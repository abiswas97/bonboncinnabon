...
- **References**: `references/task-contract.md` (Title contract rewritten to verb-first
  ...
  stage-tag family).
- **Skills**: `skills/intake/SKILL.md` (worked example) and `skills/decompose/SKILL.md`
  (decompose step) updated so their examples use verb-first titles + stage tags, not the
  old `Stage: qualifier`.
