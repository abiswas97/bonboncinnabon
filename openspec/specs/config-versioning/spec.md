# config-versioning Specification

## Purpose

Defines how butler's `config.yaml` carries a versioned schema and how every skill
that reads config reconciles the installed `config_version` against the plugin's
current schema version before using the config. This keeps config a single,
self-describing source of truth and makes config-shape changes safe across plugin
