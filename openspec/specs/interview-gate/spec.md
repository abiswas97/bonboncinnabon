# interview-gate Specification

## Purpose

Defines the blocking interview that `intake` and `decompose` MUST complete before
producing or writing any task tree. The gate ensures intent, prior progress, and
task shape are established before decomposition, and that skills carry an explicit
do-not-proceed contract so the interview is treated as a hard requirement rather
than an advisory step.
